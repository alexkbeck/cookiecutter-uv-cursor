...
bake-and-test-deploy: ## For quick publishing to cookiecutter-uv-cursor-example to test GH Actions
	@rm -rf cookiecutter-uv-cursor-example || true
	@uv run cookiecutter --no-input . --overwrite-if-exists \
		author="Alex Beck" \
		email="alexkbeck@gmail.com" \
		github_author_handle=alexkbeck \
		project_name=cookiecutter-uv-cursor-example \
		project_slug=cookiecutter_uv_example
	@cd cookiecutter-uv-cursor-example; uv sync && \
		git init -b main && \
		git add . && \
		...
		git remote add origin git@github.com:alexkbeck/cookiecutter-uv-cursor-example.git && \
